"""QLoRA-in-spirit training: int4-quantized frozen base + fp32 LoRA adapters.

The base model's Linear weights are stored as block-wise int4 (2 codes/byte)
and kept frozen; LoRA adapters train in fp32; the merged result is exported
back to an int4 checkpoint (<=8MB target).

Usage:
    uv run python -m forger.quant.qlora --ckpt checkpoints/sft/merged \
        --steps 120 --device cpu --export models/forgelm-4bit
"""
...
from forger.quant.quantize import Int4Linear, export_4bit, quantize_model_4bit, storage_size_mb
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="forger-qlora")
    parser.add_argument("--ckpt", default="checkpoints/sft/merged")
    ...
    parser.add_argument("--ckpt-dir", default="checkpoints/qlora")
    parser.add_argument("--export", default=None, help="int4 export directory (default: none)")
    ...
    n_replaced = quantize_model_4bit(model)
    print(f"quantized {n_replaced} Linear layers to int4 "
          f"(storage {storage_size_mb(model):.2f} MB)")
    ...
    replaced = apply_lora(model, lora, ("c_attn", "c_proj", "gate", "up", "down"), allowed_types=(Int4Linear,))
    n_lora = count_lora_params(model)
    print(f"LoRA on {len(replaced)} int4 layers, {n_lora:,} trainable params")
    ...
    trainer.train()
    trainer.save()
    print(f"qlora done in {time.monotonic() - start:.0f}s, final loss {trainer.loss_history[-1]:.4f}")
    ...
    if args.export:
        ...
        print(f"int4 export saved to {args.export} ({storage_size_mb(model):.2f} MB)")
    return 0
