...
| 2026-08-22 | generation crash past context_length | RoPE `cos_cached[offset:offset+t]` returns empty past ctx -> size-0 sdpa output | `GPT.forward` raises cached+new > ctx; `generate()` stops at limit (context_limited flag) |
| 2026-08-22 | Day 6: eval harness | `forger/eval/{perplexity,generation,metrics,run}.py`, `forger/model/checkpoint.py`, pinned eval corpus (500 validation stories, sha256 06aa34...), `benchmarks/eval_report.md`, ADR-06 | ppl 8.449 / bpb 1.0366 / 136K tokens; 51 tests green; commit + tag v0.2.0 |
| 2026-08-22 | Day 7: inference engine | `forger/serve/{engine,optimize}.py`, `scripts/bench_engine.py`, `benchmarks/serve_speedup.{md,json}`, ADR-07; smoke_infer refactored onto Engine | fp32 123 tok/s / 8.1ms per-token / first-token 10ms; int8 4MB; 59 tests; tag v0.2.1 |
| 2026-08-22 | torch.compile fails on Windows CPU | Inductor needs MSVC `cl.exe` (InvalidCxxCompiler) - fails at FIRST call, not compile time | bench_engine try/except around the timing + failed-status row; documented ADR-07; Day 10 ORT path will be the real speedup |
| 2026-08-22 | int8 slower than fp32 on decode (1.22x) | quantized kernel overhead on small matrices (d=256) | accepted + documented; int8 kept for 5x size (4MB <= 8MB spec); benchmark KeyError bugs (status clobber) fixed |
| 2026-08-22 | Day 8: LoRA SFT | `forger/ft/{lora,sft_data,train_sft}.py`, tests, local evidence run, ADR-08 | 75s, loss 5.08, 3.3% trainable; commit + tag v0.3.0 |
| 2026-08-22 | Day 9: QLoRA int4 | `forger/quant/{quantize,qlora}.py`, tests, `models/forgelm-4bit/` (6.86MB file, 4MB codes), ADR-09 | ppl 7.88 -> 8.11 (+2.9%); commit + tag v0.3.1 |
| 2026-08-22 | int4 dequant alloc 4.3GB | 1-D codes x (blocks,1) scale broadcast -> (blocks, n) product | reshape codes to (-1, BLOCK_SIZE) before scaling |
| 2026-08-22 | int4 model broken (ppl 734K) | `load_4bit` built fresh GPT and restored only embedding + int4 layers - RMSNorm weights stayed random (toy test passed only via identical RNG state) | export now carries full non-int4 state dict (params) |
| 2026-08-22 | double quantization drift (1.08) | `load_4bit` fed dequantized weights back into Int4Linear.__init__ (re-quantized) | `Int4Linear.from_stored` reconstructs from codes/scales directly |
| 2026-08-22 | `nn.Parameter(None)` size-0 bias | bias-less Linears (gate/up/down) got a 0-element bias parameter | only create Parameter when bias is not None |
| 2026-08-22 | dolly SFT model broken (ppl 38 stories, 68 dolly, garbage) | DOMAIN MISMATCH: 5.25M TinyStories-only base has no general knowledge; dolly KB questions -> confident garbage. Pipeline verified correct (merge diff 8e-6, zero-LoRA == baseline) | switched SFT data to story-domain instructions (TinyStories "Write a story about X") -> loss 1.96, coherent instruction-following; dolly experiment documented as the lesson |
| 2026-08-22 | kaggle_sft.py ModuleNotFoundError (2nd time) | sys.path.insert placed AFTER forger imports | moved insert before imports (same trap as kaggle_baseline.py) |
| 2026-08-22 | "skipping everything" scare in SFT run | old sft_data printed 251 skip lines (1.7%) + silent 6-min encode phase | summary skip line + encode progress in kaggle_sft.py |
| 2026-08-22 | dolly SFT model broken (ppl 38 stories, 68 dolly, garbage) | DOMAIN MISMATCH: 5.25M TinyStories-only base has no general knowledge; dolly KB questions -> confident garbage. Pipeline verified correct (merge diff 8e-6, zero-LoRA == baseline) | switched SFT data to story-domain instructions (TinyStories "Write a story about X") -> loss 1.96, coherent instruction-following; dolly experiment documented as the lesson |
| 2026-08-22 | kaggle_sft.py ModuleNotFoundError (2nd time) | sys.path.insert placed AFTER forger imports | moved insert before imports (same trap as kaggle_baseline.py) |
| 2026-08-22 | "skipping everything" scare in SFT run | old sft_data printed 251 skip lines (1.7%) + silent 6-min encode phase | summary skip line + encode progress in kaggle_sft.py |
| 2026-08-22 | Kaggle run 5 STILL memorizing (train windows: 378) | TWO stacked bugs: (1) `n = min(windows_per_story, len//ctx)` capped crops at 1 for stories < 2x ctx - the 16-window multiplier never fired; (2) ctx 256 filters stories < 257 tokens -> 91% of TinyStories discarded (avg story ~150 tokens) | windows_per_story now unconditional (16 crops per qualifying story); kaggle.yaml ctx 256 -> 128 (97% of stories qualify, 6176 windows vs 386 for 400 stories); best-eval checkpoint (best_model.pt + best_eval.json) as overfit safeguard; notebook max-stories 20000 (user must RE-IMPORT notebook - their run still showed 2000) |
